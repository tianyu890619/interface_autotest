...
class GetContactGoods(unittest.TestCase):
    def setParameters(self, casename, appKey, locale, method, v, messageFormat, goodsId, contactCode, packingId, merchantId, loginName, password, name, mobile, userType, code, message):
        ...
        self.goodsId = str(goodsId)
        self.contactCode = str(contactCode)
        self.packingId = str(packingId)
    ...
    def testgetUser(self):
        ...
        'packingId':self.packingId,
		'sign':getsign.get_sign('123456789',appKey=self.appKey,method=self.method,v=self.v,messageFormat=self.messageFormat,locale=self.locale,goodsId=self.goodsId,contactCode=self.contactCode,
                                packing=self.packingId)}
